...
import { Filter, LayoutGrid, List, ArrowRight } from 'lucide-react-native';
...
export default function ProductsContent() {
  // const { t } = useTranslation('products'); // Uncomment if translation needed
    ...
    <View className="bg-background z-10">
          ...
          <Sheet open={isFilterSheetOpen} onOpenChange={setIsFilterSheetOpen}>
            <SheetTrigger asChild>
              <Button variant="outline" size="sm" className="flex-row gap-2 border-border bg-background active:bg-accent">
                ...
                <Text className="font-medium">تصفية</Text>
              </Button>
            </SheetTrigger>
            <SheetContent side="bottom" className="h-[85%] rounded-t-2xl">
              <SheetHeader className="bb border-border pb-4 mb-2">
                <SheetTitle className="text-center">تصفية المنتجات</SheetTitle>
              </SheetHeader>
      ...
      <View className="bg-background">
        ...
        <View className="px-4 py-2 bg-secondary/20">
            ...
            {isLoading ? 'جاري البحث...' : `تم العثور على ${data?.pagination?.total || 0} منتج`}
            </Text>
        </View>
      </View>
    </View>
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <FlatList
        data={products}
        // Use key to force re-render when switching modes to apply numColumns correctly
        key={viewMode} 
...
function ProductGridSkeleton({ viewMode }: { viewMode: "grid" | "list" }) {
  ...
  const itemWidth = (width - 44) / 2; // Approximate grid width
